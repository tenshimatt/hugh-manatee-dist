...
Two options, to be decided with JWM:

**Option A — On-prem.** Server at JWM's primary location running Proxmox or similar. Best data sovereignty. Requires JWM to provide hardware (~$15–25K one-time) and basic IT handoff. Backup to a secondary location (office or cloud object storage).

**Option B — JWM-owned cloud.** Single-tenant deployment on a cloud provider (AWS/Azure/GCP) in a JWM-owned account. We manage the stack; JWM owns the infrastructure. Higher recurring cost but zero hardware procurement.

**Recommendation:** Option A if JWM has any IT operations capacity and a suitable server room. Option B if not. Hybrid possible: on-prem primary + cloud DR.
...
## 10. Commercial Model (placeholder — confirm post-Monday)

**Target build budget:** $40,000–$60,000 fixed price for Phase 1 scope.

**Assumed shape:**
- Matt (architect + delivery lead + AI gateway): ~200 hours
- Frappe developer contractor (implementation): ~350–400 hours
- Infrastructure provisioning and integration: included
- Training and documentation: included

**Excluded from build budget:**
- Hardware for on-prem hosting (if Option A)
- Cloud hosting fees (if Option B)
- LLM API usage costs (Anthropic, passed through at cost + small margin)
- Ongoing support retainer (separate agreement)
- Phase 2 scope

**Ongoing:**
- Support retainer: monthly fee covering incident response, minor enhancements, AI gateway management. Scoped post-launch.
- Phase 2 engagements: priced separately when triggered.
...
5. **Hosting preference.** On-prem at JWM's facility, cloud on JWM's account, or hybrid? Does JWM have any IT operational capacity for on-prem, or are they fully outsourced?
...
- JWM can provide or commit to infrastructure (on-prem hardware or a cloud account) without that being a project-critical blocker.
