...
// .wrangler/tmp/bundle-bFcBuv/checked-fetch.js
...
async function getOverview(request, env) {
  ...
  try {
    ...
    const data = {
      ...
      requestsToday: todayRequestsResult?.count || 0,
      averageResponseTime: Math.round(avgResponseResult?.avg || 0),
      errorRate: errorRateResult?.total > 0 ? Math.round(errorRateResult.errors / errorRateResult.total * 100 * 10) / 10 : 0,
      uniqueUsers: uniqueUsersResult?.count || 0,
      usersToday: todayUsersResult?.count || 0,
      topEndpoints: topEndpointsResult.results?.map((row) => ({
        endpoint: row.endpoint,
        calls: row.calls,
        avgTime: Math.round(row.avgTime)
      })) || [],
      recentActivity: recentActivityResult.results?.map((row) => ({
        time: row.time,
        endpoint: row.endpoint,
        status: row.status,
...
async function getEndpointMetrics(request, env) {
  ...
  try {
    const endpointsResult = await env.DB.prepare(`
        ...
        `).all();
    const data = {
      endpoints: endpointsResult.results?.map((row) => ({
        endpoint: row.endpoint,
        method: row.method,
        totalCalls: row.totalCalls,
        successRate: Math.round(row.successRate * 10) / 10,
        averageResponseTime: Math.round(row.averageResponseTime),
...
// .wrangler/tmp/bundle-bFcBuv/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-bFcBuv/middleware-loader.entry.ts
