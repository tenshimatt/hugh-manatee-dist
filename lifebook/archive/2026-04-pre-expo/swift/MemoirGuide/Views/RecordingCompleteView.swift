...
struct RecordingCompleteView: View {
    ...
    @State private var selectedChapter: ChapterEntity? = nil
                ...
                ScrollView {
                    VStack(spacing: 25) {
                        ...
                        VStack(spacing: 15) {
                            ...
                            if !chapters.isEmpty {
                                ...
                                Menu {
                                    ForEach(chapters, id: \.id) { chapter in
                                        Button("\(chapter.title ?? "Untitled Chapter")") {
                                    ...
                                    HStack {
                                        if let selected = selectedChapter {
                                            Text(selected.title ?? "Untitled Chapter")
                                                .foregroundColor(DesignSystem.textPrimary)
                                        } else {
                            ...
                            if showingCreateChapter {
                                VStack(spacing: 12) {
                                    TextField("Story title (e.g. \"My Childhood\")", text: $newChapterTitle)
                                        .font(DesignSystem.body)
                                        .padding()
                                        .frame(height: 55)
                                        .background(Color.white)
                                        ...
                                        .overlay(
                                            RoundedRectangle(cornerRadius: 12)
                                                .stroke(DesignSystem.deepNavy, lineWidth: 2)
                                        )

                                    HStack(spacing: 12) {
                                        Button("Cancel") {
                                            showingCreateChapter = false
                                            newChapterTitle = ""
                                            selectedChapter = nil
                                        }
                                        .font(DesignSystem.body)
                                        ...
                                        Button("Create") {
                                            if !newChapterTitle.isEmpty {
                                                ...
                                                let newChapter = coreDataManager.createChapter(
                                        ...
                                        .frame(maxWidth: .infinity)
                                        .frame(height: 44)
                                        .background(newChapterTitle.isEmpty ? Color.gray : DesignSystem.deepNavy)
                                        .cornerRadius(12)
                                        .disabled(newChapterTitle.isEmpty)
                                    }
                                }
                            } else {
                                Button(action: {
                        ...
                        VStack(spacing: 15) {
                            Button(action: saveStory) {
                                Text("Save This Memory")
                                    ...
                                    .background(canSave ? DesignSystem.deepNavy : Color.gray)
    ...
    func saveStory() {
        ...
        Task { @MainActor in
            do {
                ...
                // Link session to chapter if one was selected
                if let chapter = selectedChapter {
                    session.chapter = chapter
                    chapter.addSession(session)
                }
