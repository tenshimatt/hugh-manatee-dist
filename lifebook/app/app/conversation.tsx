...
export default function Conversation() {
  ...
  const conversation = useConversation({
    onConnect: ({ conversationId }: { conversationId: string }) => {
      ...
      // Deliver the deterministic opener as context so Hugh says it verbatim.
      // (The agent's "first message" is configured empty server-side.)
      const opener = firstTurnRef.current;
      if (opener) {
        conversation.sendContextualUpdate(`Say this as your next turn: "${opener}"`);
      }
    },
    onDisconnect: () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setStatus("ready");
        // Private WebRTC: conversationToken without agentId (SDK union type).
...
const styles = StyleSheet.create({
  ...
  endButtonText: { color: colors.surface, fontSize: fontSize.bodyLarge, fontWeight: "600" },
});
