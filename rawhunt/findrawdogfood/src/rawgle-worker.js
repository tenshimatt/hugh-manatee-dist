...
async function handleSearch(params, env) {
  ...
  // Allow empty queries to return all results
  ...
  try {
    ...
    const searchPattern = query.trim() ? `%${query}%` : '%';
...
async function handleSupplierDetails(params, env) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: 'Failed to fetch supplier details' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' }
    });
  }
