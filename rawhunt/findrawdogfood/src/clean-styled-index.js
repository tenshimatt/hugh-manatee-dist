// FindRawDogFood - Clean Professional Website
export default {
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .newsletter-btn:hover { background: #7A6348; }
...
<body>
    ...
    <script>
        ...
        async function searchSuppliers(event) {
            ...
            resultsDiv.innerHTML = '<div style="padding: 2rem; text-align: center; color: #666;">Searching suppliers...</div>';
            ...
            try {
                ...
                if (data.success && data.suppliers.length > 0) {
                    let html = '<h3>Found ' + data.suppliers.length + ' suppliers near "' + location + '"</h3>';
                    html += '<div class="suppliers-list">';
                    
                    data.suppliers.forEach(supplier => {
                        html += '<div class="supplier-card">';
                        html += '<div class="supplier-header">';
                        html += '<h4>' + supplier.name + '</h4>';
                        html += '<div class="supplier-rating">';
                        if (supplier.rating) html += '⭐ ' + supplier.rating;
                        if (supplier.user_ratings_total) html += ' (' + supplier.user_ratings_total + ')';
                        html += '</div></div>';
                        html += '<div class="supplier-details">';
                        html += '<p>' + (supplier.address || supplier.city + ', ' + supplier.state) + '</p>';
                        if (supplier.phone_number) html += '<p>📞 ' + supplier.phone_number + '</p>';
                        if (supplier.website) html += '<p><a href="' + supplier.website + '" target="_blank">🌐 Visit Website</a></p>';
                        html += '</div></div>';
                    });
                    ...
                    resultsDiv.innerHTML = '<div style="padding: 2rem; text-align: center; color: #666;"><h3>No suppliers found near "' + location + '"</h3><p>Try searching for a nearby city.</p></div>';
                }
            } catch (error) {
                resultsDiv.innerHTML = '<div style="padding: 2rem; text-align: center; color: #d32f2f;">Error searching suppliers. Please try again.</div>';
            }
        }
...
function getBlogPage() {
  return `<!DOCTYPE html><html><head><title>Blog - FindRawDogFood</title></head><body><h1>Blog</h1><p>Coming soon...</p></body></html>`;
}

function getBlogPost() {
  return `<!DOCTYPE html><html><head><title>The Raw Truth - FindRawDogFood</title></head><body><h1>The Raw Truth: Why Dogs Thrive on a BARF Diet</h1><p>Full article coming soon...</p></body></html>`;
}
