// Working SEO-Optimized FindRawDogFood Worker (Debugged)
export default {
  async fetch(request, env) {
    const url = new URL(request.url);
    const path = url.pathname;

    // SEO Routes (simple ones first)
    if (path === '/robots.txt') {
...
async function generateSitemap(origin, env) {
  try {
    ...
    // Try to get cities from database, fallback to static list
    if (env.FINDRAWDOGFOOD_DB) {
      try {
        ...
        console.log('Database error in sitemap, using fallback');
      }
    }
    
    // Fallback cities if database unavailable
    if (cities.length === 0) {
      cities = [
        { city: 'London' }, { city: 'Manchester' }, { city: 'Birmingham' },
        { city: 'Leeds' }, { city: 'Glasgow' }, { city: 'Liverpool' },
        { city: 'Newcastle' }, { city: 'Sheffield' }, { city: 'Bristol' },
        { city: 'Edinburgh' }
      ];
    }
...
${cities.map(city => `  <url>
    <loc>${origin}/location/${encodeURIComponent((city.city || city).toLowerCase().replace(/\s+/g, '-'))}</loc>
    <lastmod>${new Date().toISOString().split('T')[0]}</lastmod>
        ...
        <div class="supplier-grid">
            ...
            `).join('') : `
                <div style="grid-column: 1/-1; text-align: center; padding: 40px;">
                    <h3>Suppliers database loading...</h3>
                    <p>Please check back soon for suppliers in ${cityName}</p>
                </div>
            `}
        </div>
    </div>
...
<body>
    ...
    <script>
        ...
        async function search() {
            ...
            resultsContent.innerHTML = '<div style="text-align: center; padding: 40px;">🔍 Searching...</div>';
            ...
            try {
                ...
                resultsContent.innerHTML = '<div style="text-align: center; padding: 40px; color: #e74c3c;"><h3>Search Error</h3><p>Database is loading. Please try again in a moment.</p></div>';
            }
        }
