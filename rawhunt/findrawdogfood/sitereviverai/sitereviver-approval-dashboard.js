...
// Business rules for filtering prospects
...
function getPriorityRank(analysisScore) {
    ...
    if (analysisScore <= 10) return { rank: 1, label: 'CRITICAL', value: '$20k-$30k' };
    if (analysisScore <= 30) return { rank: 2, label: 'URGENT', value: '$15k-$25k' };
    if (analysisScore <= 50) return { rank: 3, label: 'HIGH PRIORITY', value: '$10k-$18k' };
    if (analysisScore <= 70) return { rank: 4, label: 'MEDIUM', value: '$8k-$12k' };
    return { rank: 5, label: 'LOW PRIORITY', value: '$5k-$8k' };
}

async function getQualifiedProspects(env) {
    try {
        const { results } = await env.DB.prepare(`
            SELECT id, name, website, phone_number, city, state, rating
            FROM suppliers 
        ...
        for (const supplier of results) {
            ...
            const analysisScore = Math.round((supplier.rating || 1) * 20);
            const priority = getPriorityRank(analysisScore);
            
            if (priority.rank >= 5) continue;
            
            const domain = new URL(supplier.website).hostname.replace('www.', '');
        ...
        return qualified.sort((a, b) => a.analysis_score - b.analysis_score);
    } catch (error) {
...
<head>
    ...
    <style>
        ...
        .email-preview { background: #f8f9fa; padding: 20px; border-radius: 10px; margin: 20px 0; border: 1px solid #e0e0e0; max-height: 200px; overflow-y: auto; font-size: 0.9rem; line-height: 1.5; white-space: pre-line; }
...
export default {
    async fetch(request, env, ctx) {
        ...
        try {
            ...
            return new Response(`Error: ${error.message}\n\nThis worker needs D1 database binding configured.`, {
                status: 500,
