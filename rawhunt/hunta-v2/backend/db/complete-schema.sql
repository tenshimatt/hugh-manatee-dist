...
-- Hunt logs table for GPS tracking and hunt management
CREATE TABLE IF NOT EXISTS hunt_logs (
    id TEXT PRIMARY KEY DEFAULT (hex(randomblob(16))),
    user_id TEXT NOT NULL,
    hunt_date DATE NOT NULL,
    location TEXT NOT NULL,
    dogs_present TEXT, -- JSON array of dog IDs/names
    duration_minutes INTEGER DEFAULT 0,
    success_rating INTEGER CHECK (success_rating BETWEEN 1 AND 5) DEFAULT 3,
    weather_conditions TEXT, -- JSON object with weather data
    gps_route TEXT, -- JSON object with waypoints and route data
    game_harvested TEXT, -- JSON array of harvest data
    equipment_used TEXT, -- JSON array of equipment used
    notes TEXT,
...
CREATE TABLE IF NOT EXISTS training_sessions (
    ...
    duration_minutes INTEGER DEFAULT 0,
    performance_rating INTEGER CHECK (performance_rating BETWEEN 1 AND 5) DEFAULT 3,
    skills_practiced TEXT, -- JSON array of skills worked on
    improvements_noted TEXT,
    challenges TEXT,
    notes TEXT,
    weather_conditions TEXT, -- JSON object
    location TEXT,
    videos TEXT, -- JSON array of video URLs/metadata
    photos TEXT, -- JSON array of photo URLs
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (dog_id) REFERENCES dogs(id)
...
CREATE INDEX IF NOT EXISTS idx_hunt_logs_user_id ON hunt_logs(user_id);
CREATE INDEX IF NOT EXISTS idx_hunt_logs_date ON hunt_logs(hunt_date);
...
CREATE INDEX IF NOT EXISTS idx_training_goals_dog_id ON training_goals(dog_id);
CREATE INDEX IF NOT EXISTS idx_training_goals_status ON training_goals(status);
