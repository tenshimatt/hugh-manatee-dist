-- Rawgle platform specific tables
...
-- Rawgle suppliers table (separate from existing suppliers)
CREATE TABLE IF NOT EXISTS rawgle_suppliers (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER,
    name TEXT NOT NULL,
    description TEXT,
...
CREATE TABLE IF NOT EXISTS reviews (
    ...
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(id),
    FOREIGN KEY (supplier_id) REFERENCES rawgle_suppliers(id),
...
CREATE INDEX IF NOT EXISTS idx_rawgle_suppliers_location ON rawgle_suppliers(location_latitude, location_longitude);
CREATE INDEX IF NOT EXISTS idx_rawgle_suppliers_category ON rawgle_suppliers(category);
CREATE INDEX IF NOT EXISTS idx_rawgle_suppliers_rating ON rawgle_suppliers(rating_average);
