                    ...
                    <div class="analysis-container" id="analysisContainer">
                        ...
                        <div class="analysis-results" id="analysisResults" style="display: none;">
                            <div class="analysis-summary">
                                <h3>\u{1F9E0} Project Analysis</h3>
                                <div id="projectAnalysis"></div>
                            </div>
                            
                            <div class="selected-agents">
                                <h3>\u{1F916} Recommended Agent Team</h3>
                            ...
                            <div class="analysis-reasoning">
                                <h3>\u{1F4A1} Selection Reasoning</h3>
    ...
    <script>
        ...
        function displayCodebaseAnalysis(result) {
            ...
            if (config.projectType === 'fix') {
                ...
                const summaryHTML = '<h3 style="color: var(--accent-blue); margin-bottom: 15px;">\u{1F50D} Codebase Analysis</h3>' +
                ...
                const summaryHTML = '<h3 style="color: var(--accent-blue); margin-bottom: 15px;">\u{1F680} Project Foundation</h3>' +
            ...
            const techHTML = '<h3 style="color: var(--accent-green); margin-bottom: 15px;">\u{1F4CA} Technical Analysis</h3>' +
                '<div style="background: var(--bg-primary); padding: 15px; border-radius: 8px;">' +
...
echo "\u{1F680} Starting automated Tmux Orchestrator setup for ${sessionName}..."
...
make_executable() {
    chmod +x "$1"
    echo "\u{1F527} Made executable: $1"
}

# Function to fetch and apply configuration
apply_config() {
    echo "\u{1F4E5} Fetching configuration for ${sessionName}..."
...
generate_all_scripts() {
    ...
    echo "\u{1F528} Generating all project scripts..."
    ...
    echo "\u{1F4DD} Creating quick commands reference..."
...
launch_orchestrator() {
    echo "\u{1F3AC} Launching Tmux Orchestrator..."
    ...
    echo "\u{1F680} Starting tmux session: ${sessionName}"
...
main() {
    ...
    echo ""
    echo "\u{1F389} Setup complete! All files created:"
    ...
    read -p "\u{1F680} Launch the orchestrator now? (Y/n): " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Nn]$ ]]; then
...
__name(generateAutomatedSetup, "generateAutomatedSetup");
...
__name(generateAgentSetupCommands, "generateAgentSetupCommands");
...
function generateSetupGuide(config) {
  ...
  return `
    <div style="font-family: monospace; line-height: 1.6;">
      <h2 style="color: var(--accent-blue); margin-bottom: 20px;">\u{1F680} Complete Setup Guide</h2>
      
      <div style="background: var(--bg-primary); padding: 20px; border-radius: 10px; margin-bottom: 20px;">
        <h3 style="color: var(--accent-green); margin-bottom: 15px;">\u{1F4CB} Project Overview</h3>
        <p><strong>Project:</strong> ${config.projectName}</p>
        <p><strong>Type:</strong> ${config.projectType === "fix" ? "\u{1F527} Fix Project" : "\u{1F680} New Project"}</p>
        <p><strong>Path:</strong> ${config.projectPath}</p>
      ...
      <div style="background: var(--bg-primary); padding: 20px; border-radius: 10px; margin-bottom: 20px;">
        <h3 style="color: var(--accent-orange); margin-bottom: 15px;">\u{1F916} Claude Code Agent Setup</h3>
...
curl -s https://tmux-orchestrator-ui-prod.findrawdogfood.workers.dev/api/setup/${sessionName} | bash
        ...
        <p style="color: var(--text-secondary); font-size: 0.9em;">
          \u{1F3AF} <strong>Claude Code agents:</strong> Use this command to automatically set up the complete orchestrator system.<br/>
      ...
      <div style="background: var(--bg-primary); padding: 20px; border-radius: 10px; margin-bottom: 20px;">
        <h3 style="color: var(--accent-blue); margin-bottom: 15px;">\u{1F6E0}\uFE0F Manual Setup Steps</h3>
...
nano launch_${sessionName}.sh
          ...
          <p style="color: var(--text-secondary); font-size: 0.9em;">
            \u{1F4DD} Copy the "Launcher Script" tab contents into this file
          </p>
        </div>
...
nano project_spec.md
          ...
          <p style="color: var(--text-secondary); font-size: 0.9em;">
            \u{1F4DD} Copy the "Project Spec" tab contents into this file
          </p>
        </div>
...
./launch_${sessionName}.sh
      ...
      <div style="background: var(--bg-primary); padding: 20px; border-radius: 10px; margin-bottom: 20px;">
        <h3 style="color: var(--accent-red); margin-bottom: 15px;">\u{1F3AF} What Happens When You Launch</h3>
      ...
      <div style="background: var(--bg-primary); padding: 20px; border-radius: 10px; margin-bottom: 20px;">
        <h3 style="color: var(--accent-blue); margin-bottom: 15px;">\u{1F3AE} Control Commands</h3>
...
__name(generateSetupGuide, "generateSetupGuide");
...
echo "SCHEDULED TO RUN AT: $RUN_TIME (in $MINUTES minutes from $CURRENT_TIME)"`;
  return `
    <div style="margin-bottom: 20px;">
      <h3 style="color: var(--accent-blue); margin-bottom: 15px;">\u{1F4C2} send-claude-message.sh</h3>
    ...
    <div style="background: rgba(100, 181, 246, 0.1); border: 1px solid var(--accent-blue); padding: 15px; border-radius: 10px;">
      <h4 style="color: var(--accent-blue); margin-bottom: 10px;">\u{1F4A1} Usage Instructions</h4>
      ...
      <p style="margin-top: 10px; color: var(--text-secondary);">
        \u{1F4BE} Save both scripts to your project directory and make them executable with: <code>chmod +x *.sh</code>
      </p>
    </div>
  `;
}
__name(generateUtilityScripts, "generateUtilityScripts");
...
__name(analyzeRealCodebase, "analyzeRealCodebase");
function analyzeRealExistingCodebase(projectData, fileList, packageJson, readmeContent) {
  ...
  analysis.techStack = {
    primary: [.../* @__PURE__ */ new Set([...detectedTech.frameworks, ...packageInfo.frameworks])],
    languages: [.../* @__PURE__ */ new Set([...detectedTech.languages, ...packageInfo.languages])],
    dependencies: packageInfo.dependencies.slice(0, 10),
    // Top 10 deps
    buildTools: [.../* @__PURE__ */ new Set([...detectedTech.buildTools, ...packageInfo.buildTools])]
  };
...
__name(analyzeRealExistingCodebase, "analyzeRealExistingCodebase");
function analyzeFileList(fileList) {
  ...
  if (extCounts.css > 0)
    analysis.languages.push("CSS");
  if (extCounts.scss > 0 || extCounts.sass > 0)
    analysis.languages.push("SCSS");
  if (extCounts.html > 0)
    analysis.languages.push("HTML");
  if (extCounts.py > 0)
  ...
  if (fileNames.some((f) => f.includes("webpack.config")))
    analysis.buildTools.push("Webpack");
  if (fileNames.some((f) => f.includes("rollup.config")))
    analysis.buildTools.push("Rollup");
  if (fileNames.some((f) => f.includes("babel.config")))
    analysis.buildTools.push("Babel");
...
__name(analyzeFileList, "analyzeFileList");
function analyzePackageJson(packageJson) {
  ...
  try {
    ...
    if (allDeps.babel || allDeps["@babel/core"])
      analysis.buildTools.push("Babel");
    if (allDeps.esbuild)
      analysis.buildTools.push("ESBuild");
    if (allDeps.wrangler)
      analysis.buildTools.push("Wrangler");
  } catch (error) {
...
__name(analyzePackageJson, "analyzePackageJson");
function analyzeReadme(readmeContent) {
  ...
  analysis.hasGoodDocs = analysis.hasInstallInstructions && (analysis.hasApiDocs || analysis.hasExamples) && content.length > 500;
  return analysis;
}
__name(analyzeReadme, "analyzeReadme");
function categorizeProjectSize(fileCount) {
  if (fileCount < 20)
    return "Small (< 20 files)";
  if (fileCount < 50)
    return "Small to Medium (20-50 files)";
  if (fileCount < 100)
    return "Medium (50-100 files)";
  if (fileCount < 200)
    return "Medium to Large (100-200 files)";
  if (fileCount < 500)
    return "Large (200-500 files)";
  return "Very Large (500+ files)";
}
__name(categorizeProjectSize, "categorizeProjectSize");
function assessCodeQuality(fileList, packageJson) {
  ...
  if (fileList.some((f) => f.includes(".test.") || f.includes(".spec.")))
    score += 2;
  if (fileList.some((f) => f.includes("eslint") || f.includes(".eslintrc")))
    score += 1;
  if (fileList.some((f) => f.includes("prettier") || f.includes(".prettierrc")))
    score += 1;
  if (fileList.some((f) => f.includes("config") && !f.includes("node_modules")))
    score += 1;
  if (packageJson) {
    try {
      const pkg = typeof packageJson === "string" ? JSON.parse(packageJson) : packageJson;
...
__name(assessCodeQuality, "assessCodeQuality");
...
__name(identifyIssues, "identifyIssues");
...
__name(analyzeCodebase, "analyzeCodebase");
...
__name(analyzeExistingCodebase, "analyzeExistingCodebase");
...
__name(generateMockFileListFromProject, "generateMockFileListFromProject");
...
__name(generateMockPackageJson, "generateMockPackageJson");
...
__name(generateMockReadme, "generateMockReadme");
...
__name(generateFeatureRecommendations, "generateFeatureRecommendations");
...
function selectOptimalAgents(projectType, analysis, codebaseInsights = null) {
  const orchestratorAgents = [
    { id: "orchestrator", name: "Orchestrator", role: "High-level coordination without implementation details", icon: "\u{1F451}", required: true }
  ];
  const projectManagerAgents = [
    { id: "project-manager", name: "Project Manager", role: "Quality-focused team coordination, meticulous testing", icon: "\u{1F4CB}", required: false }
  ];
  const developerAgents = [
    { id: "developer", name: "Developer", role: "Implementation and technical decisions", icon: "\u{1F468}\u200D\u{1F4BB}", required: false },
    { id: "frontend-dev", name: "Frontend Developer", role: "UI/UX implementation", icon: "\u{1F3A8}", required: false },
  ...
  const devopsAgents = [
    { id: "devops", name: "DevOps Engineer", role: "Infrastructure and deployment", icon: "\u{1F680}", required: false }
  ];
  const codeReviewAgents = [
    { id: "code-reviewer", name: "Code Reviewer", role: "Security and best practices", icon: "\u{1F50D}", required: false }
  ];
  const researchAgents = [
    { id: "researcher", name: "Researcher", role: "Technology evaluation", icon: "\u{1F52C}", required: false }
  ];
  const docAgents = [
    { id: "doc-writer", name: "Documentation Writer", role: "Technical documentation", icon: "\u{1F4DD}", required: false }
  ];
  const selected = [];
  ...
  if (codebaseInsights) {
    if (projectType === "fix") {
      ...
      const totalFeatures = (codebaseInsights.recommendations.essential?.length || 0) + (codebaseInsights.recommendations.optional?.length || 0);
      if (totalFeatures > 12) {
        adjustedComplexity = "high";
...
__name(selectOptimalAgents, "selectOptimalAgents");
...
__name(getEstimatedDuration, "getEstimatedDuration");
function generateReasoning(projectType, analysis, selectedAgents, codebaseInsights = null) {
  ...
  if (analysis.complexity === "low" || analysis.domains.length <= 2) {
    reasons.push(`\u{1F3D7}\uFE0F Small Project Structure: Applied tmux orchestrator rules for simple projects.`);
    reasons.push(`\u{1F451} Orchestrator: Required for all projects - maintains high-level oversight without implementation details.`);
    reasons.push(`\u{1F4CB} Project Manager: Ensures quality standards and coordinates the development team.`);
    reasons.push(`\u{1F468}\u200D\u{1F4BB} Developer: Handles implementation and technical decisions for the project scope.`);
  } else if (analysis.complexity === "medium" || analysis.domains.length <= 4) {
    reasons.push(`\u{1F3D7}\uFE0F Medium Project Structure: 2 Developers + PM + QA following tmux orchestrator patterns.`);
    reasons.push(`\u{1F451} Orchestrator: Required coordination role - monitors all agents and makes architectural decisions.`);
    reasons.push(`\u{1F4CB} Project Manager: Meticulous about testing and verification, enforces quality standards.`);
    const devCount = selectedAgents.filter((a) => a.role.includes("Developer")).length;
    if (devCount >= 2) {
      reasons.push(`\u{1F468}\u200D\u{1F4BB} Two specialized developers: Frontend and backend specialists based on identified domains (${analysis.domains.join(", ")}).`);
    }
    reasons.push(`\u2705 QA Engineer: Essential for medium complexity projects to ensure comprehensive testing.`);
  } else {
    reasons.push(`\u{1F3D7}\uFE0F Large Project Structure: Lead + Multiple Devs + PM + QA + DevOps for high complexity.`);
    reasons.push(`\u{1F451} Orchestrator: Critical oversight role - coordinates between multiple teams without getting into implementation.`);
    reasons.push(`\u{1F4CB} Project Manager: Acts as lead coordinator, ensuring quality and managing complex team communication.`);
    reasons.push(`\u{1F468}\u200D\u{1F4BB} Specialized Development Team: Multiple developers with domain expertise for comprehensive coverage.`);
    reasons.push(`\u2705 QA Engineer: Mandatory for large projects to maintain quality standards.`);
    reasons.push(`\u{1F680} DevOps Engineer: Required for high complexity projects to handle infrastructure and deployment.`);
  }
  if (selectedAgents.length > 3) {
    reasons.push(`\u{1F4E1} Hub-and-Spoke Communication: PM coordinates all developer communication to prevent n\xB2 complexity growth.`);
  }
  if (analysis.domains.includes("search") || analysis.domains.includes("database")) {
    reasons.push(`\u{1F50D} Backend Specialist: Selected for search/database optimization expertise.`);
  }
  if (analysis.domains.includes("frontend")) {
    reasons.push(`\u{1F3A8} Frontend Specialist: UI/UX implementation for user-facing components.`);
  }
  if (analysis.domains.includes("security")) {
    reasons.push(`\u{1F512} Security Review: Code reviewer added for security vulnerability assessment.`);
  }
  if (codebaseInsights) {
    if (projectType === "fix") {
      reasons.push(`\u{1F50D} Codebase Analysis: Found ${codebaseInsights.analysis.issuesFound} issues in ${codebaseInsights.analysis.projectSize} project with ${codebaseInsights.analysis.codeQuality} code quality.`);
      reasons.push(`\u{1F6E0}\uFE0F Tech Stack: Working with ${codebaseInsights.techStack.primary.join(", ")} - team selected for specific technology expertise.`);
    } else {
      const totalFeatures = (codebaseInsights.recommendations.essential?.length || 0) + (codebaseInsights.recommendations.optional?.length || 0);
      ...
      reasons.push(`\u{1F3D7}\uFE0F Architecture: ${codebaseInsights.analysis.architecture} pattern recommended with ${codebaseInsights.techStack.recommended.join(", ")} stack.`);
    }
  }
  reasons.push(`\u{1F48E} Quality Focus: All agents follow tmux orchestrator principles - no shortcuts, comprehensive testing, architectural consistency.`);
  return reasons;
}
